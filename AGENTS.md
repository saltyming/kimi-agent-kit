...
- `kimi-agent-kit--task-execution.md`: the execution loop (Understand, Plan, Execute) and the scope and state gates GATE-SCOPE-CONFIRM, GATE-DEVIATION, GATE-GIT.
- `kimi-agent-kit--delegation.md`: the delegation loop and GATE-DELEGATE.
- `kimi-agent-kit--palette.md`: the product-intent outer loop, active only in projects that contain `_palette/`, and the palette meaning of every gate.
- `kimi-agent-kit--aside.md`: when to ask the `aside` MCP server for a read-only second opinion from another model family.
- `kimi-agent-kit--dispatch.md`: when to hand an execution step to the `dispatch` MCP server, and GATE-DISPATCH.
- `kimi-agent-kit--git-workflow.md`: how the user's git preferences (signing, attribution, commit and PR format) are read, asked for, and recorded.
...
**INV-GATE-1 — Write-capable delegation is gated.** Read-only delegates are free: use them without asking, since they reduce your context cost. Do not spawn a write-capable delegate until you have told the user the mechanism, the rough cost and scale, and the files it will write, and the user has agreed (GATE-DELEGATE in `kimi-agent-kit--delegation.md`). The gate follows what the delegate can do, not the prompt you plan to send it. One exception: when the user has configured an auto-approval policy for a specific mechanism (dispatch's execution policy), that policy governs that mechanism.
...
2. **Delegation loop** (`kimi-agent-kit--delegation.md`): when work fans out to delegates. `aside` (consult) and `dispatch` (execute externally) are its MCP-backed surfaces.
3. **palette outer loop** (`kimi-agent-kit--palette.md`): backlog, slice, hand off, review, across sessions. Active only when the project contains `_palette/`. It wraps the execution loop and feeds it one approved story at a time.
...
3. **Execute**: implement every change in the approved scope (INV-SCOPE-1), using Kimi's native edit/write tools.
