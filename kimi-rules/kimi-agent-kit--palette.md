...
- **GATE-DEVIATION.** Once a story passes hand-off, its `Done when` and `Not this story` are the approved spec. Moving unmet criteria into the backlog is not a way around the gate. Plain ambiguity follows the clarification heuristic in `AGENTS.md`.
