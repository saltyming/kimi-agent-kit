...
1. Read `$KIMI_CODE_HOME/rules/kimi-agent-kit--aside-prefs.md` before any aside call. It gives the preferred backend, the default model, reasoning effort, and fallback chain, and the auto-call policy. A current-turn instruction from the user outranks it.
