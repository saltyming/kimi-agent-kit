...
**palette note.** If the step implements a palette story, its `objective` / `acceptance` come from that story's *approved* acceptance criteria (post-approval), not from the raw `story-*.rst` file. The backlog proposes, approval authorizes, and the dispatch spec carries the authorized scope. See `kimi-agent-kit--palette.md`.
